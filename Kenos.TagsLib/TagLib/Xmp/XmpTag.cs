...
using Kenos.TagsLib.TagLib.Image;
using Kenos.TagsLib.TagLib.IFD.Entries;
...
#region Private Methods
		...
		private XmpNode ParseRDF (XmlNode rdf_node, TagLib.File file)
		{
			...
			foreach (XmlNode node in rdf_node.ChildNodes) {
				...
				if (XmlNodeExtensions.Is(node, RDF_NS, DESCRIPTION_URI)) {
		...
		{
            if (!XmlNodeExtensions.IsNodeElement(node))
				throw new CorruptFileException ("Unexpected node found, invalid RDF?");

            if (XmlNodeExtensions.Is(node, RDF_NS, SEQ_URI))
            {
				parent.Type = XmpNodeType.Seq;
            }
            else if (XmlNodeExtensions.Is(node, RDF_NS, ALT_URI))
            {
				parent.Type = XmpNodeType.Alt;
            }
            else if (XmlNodeExtensions.Is(node, RDF_NS, BAG_URI))
            {
				parent.Type = XmpNodeType.Bag;
            }
            else if (XmlNodeExtensions.Is(node, RDF_NS, DESCRIPTION_URI))
            {
			...
			foreach (XmlAttribute attr in node.Attributes) {
                if (XmlNodeExtensions.In(attr, XMLNS_NS))
					continue;
                if (XmlNodeExtensions.Is(attr, RDF_NS, ID_URI) || XmlNodeExtensions.Is(attr, RDF_NS, NODE_ID_URI) || XmlNodeExtensions.Is(attr, RDF_NS, ABOUT_URI))
					continue;
                if (XmlNodeExtensions.Is(attr, XML_NS, LANG_URI))
		...
		private void ParsePropertyElement (XmpNode parent, XmlNode node)
		{
			...
			foreach (XmlAttribute attr in node.Attributes) {
                ...
                if (!XmlNodeExtensions.Is(attr, XML_NS, LANG_URI) && !XmlNodeExtensions.Is(attr, RDF_NS, ID_URI) && !XmlNodeExtensions.In(attr, XMLNS_NS))
					has_other = true;
			}
			...
			if (count > 3) {
				...
				if (!has_other) {
					...
					foreach (XmlAttribute attr in node.Attributes) {
                        ...
                        if (XmlNodeExtensions.Is(attr, RDF_NS, DATA_TYPE_URI))
                        {
							ParseLiteralPropertyElement (parent, node);
                        }
                        else if (!XmlNodeExtensions.Is(attr, RDF_NS, PARSE_TYPE_URI))
                        {
							ParseEmptyPropertyElement (parent, node);
		...
		{
            if (!XmlNodeExtensions.IsPropertyElement(node))
			...
			foreach (XmlAttribute attr in node.Attributes) {
                ...
                else if (XmlNodeExtensions.Is(attr, RDF_NS, ID_URI) || XmlNodeExtensions.In(attr, XMLNS_NS))
                {
					continue;
				}
		...
		private void ParseEmptyPropertyElement (XmpNode parent, XmlNode node)
		{
			...
			foreach (XmlAttribute a in node.Attributes) {
                if (XmlNodeExtensions.Is(a, RDF_NS, ID_URI) || XmlNodeExtensions.Is(a, RDF_NS, NODE_ID_URI))
                {
					continue;
                }
                else if (XmlNodeExtensions.In(a, XMLNS_NS))
                {
					continue;
                }
                else if (XmlNodeExtensions.Is(a, XML_NS, LANG_URI))
                {
		...
		private XmpNode CreateTextPropertyWithQualifiers (XmlNode node, string value)
		{
			...
			foreach (XmlAttribute attr in node.Attributes) {
                if (XmlNodeExtensions.In(attr, XMLNS_NS))
					continue;
                if (XmlNodeExtensions.Is(attr, RDF_NS, VALUE_URI) || XmlNodeExtensions.Is(attr, RDF_NS, RESOURCE_URI))
